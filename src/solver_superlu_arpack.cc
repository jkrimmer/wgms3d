...
namespace
{
    ...
    void GSTRF (superlu_options_t *a, SuperMatrix *AC,
		...
		SuperMatrix *k, SuperMatrix *l, GlobalLU_t * Glu, SuperLUStat_t *m, int *n)
    {
	if(AC->Dtype == SLU_D) {
	    dgstrf(a,AC,d,e,f,g,h,i,j,k,l,Glu,m,n);
	} else if(AC->Dtype == SLU_Z) {
	    zgstrf(a,AC,d,e,f,g,h,i,j,k,l,Glu,m,n);
	}
    }
    ...
    Create_CompCol_Matrix (SuperMatrix *a, int b, int c, int d, std::complex<double> *e,
			   int *f, int *g, Stype_t h, Mtype_t j)
    {
	zCreate_CompCol_Matrix (a, b, c, d, reinterpret_cast <doublecomplex*> (e), f, g, h, SLU_Z, j);
    }
